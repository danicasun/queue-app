...
export default function ActivityCard({ activity, onSave, onOpenComments }) {
  const [saved, setSaved] = useState(activity.saved || false);
  ...
  return (
    <div className="rounded-2xl bg-white shadow-[0_1px_3px_rgba(0,0,0,0.04),0_4px_12px_rgba(0,0,0,0.03)] p-4">
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          <span className="text-[13px] text-gray-400">
            <span className="font-medium text-gray-600">{activity.name}</span> is curious about
          </span>
        </div>
      </div>
      ...
      {activity.context && (
        <p className="text-[13px] text-gray-400 mb-2.5 line-clamp-1">{activity.context}</p>
      )}
